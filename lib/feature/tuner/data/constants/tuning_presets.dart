import 'package:fluttertuner/feature/tuner/data/models/note_model.dart';
import 'package:fluttertuner/feature/tuner/data/models/tuning_model.dart';

class TuningPresets {
  static const standardTuning = TuningModel(
    name: 'Standard E',
    notes: [
      NoteModel(name: 'E', frequency: 82.41),
      NoteModel(name: 'A', frequency: 110.00),
      NoteModel(name: 'D', frequency: 146.83),
      NoteModel(name: 'G', frequency: 196.00),
      NoteModel(name: 'B', frequency: 246.94),
      NoteModel(name: 'E', frequency: 329.63),
    ],
  );

  static const dropDTuning = TuningModel(
    name: 'Drop D',
    notes: [
      NoteModel(name: 'D', frequency: 73.42),
      NoteModel(name: 'A', frequency: 110.00),
      NoteModel(name: 'D', frequency: 146.83),
      NoteModel(name: 'G', frequency: 196.00),
      NoteModel(name: 'B', frequency: 246.94),
      NoteModel(name: 'E', frequency: 329.63),
    ],
  );

  static const defaultTunings = [standardTuning, dropDTuning];
}
